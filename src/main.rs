...
// struct, list, vector, enum, derive
...
impl Board {
    ...
    fn can_place_ship(&mut self, row:usize, col:usize, size:usize, direction:bool) -> bool {
        if col + size > BOARD_SIZE || row + size > BOARD_SIZE {
            return false;
        }
        if direction {         
            for i in 0..size {
...
fn get_player_input() -> (usize, usize) {
    loop {
        print!("\x1b[;37mEner coordinates to fire (row, col): \x1b[0m");
        ...
        if coordinates.len() == 2 && coordinates[0] < BOARD_SIZE && coordinates[1] < BOARD_SIZE {
            ...
            println!("\x1b[1;31mInvalid input. Please enter row and column numbers separated by comma")
        }
